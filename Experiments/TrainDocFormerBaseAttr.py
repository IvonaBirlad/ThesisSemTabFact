...
if __name__ == "__main__":
    pl_docformer, pl_datamodule = main()

model_path = os.path.join('./models', os.listdir('./models')[0])
model = pl_docformer.load_from_checkpoint(model_path)
model_res = model.resnet.load_state_dict(model.model.extract_feature.visual_feature.state_dict())
model_res = model_res.eval()
model_res = model_res.to(device)

idx = 30
sample_encoding = val_ds[idx]

sample_path = train_df['table_name'].tolist()[idx]
image = Image.open(sample_path).convert("RGB")
input_statement = train_df['statement'].tolist()[idx]
...
output = model(input_img, input_statement)
output = F.softmax(output, dim=1)
prediction_score, predicted_label = torch.topk(output, 1)
...
attributions_ig = integrated_gradients.attribute(input, target=predicted_label, n_steps=200)
...
attributions_plot = viz.visualize_image_attr(np.transpose(attributions_ig.squeeze().cpu().detach().numpy(), (1,2,0)),
...
attributions_plot.savefig('attrib.png')
...
# 4. Captum Library Tutorials: https: // captum.ai / tutorials / Resnet_TorchVision_Interpret
