...
PNG_PATH_MAN = "png_data/data_manual.csv"
PNG_PATH_AUTO = "png_data_auto/data_manual_auto.csv"
...
data_test = data_test[data_test.label != 2] #remove unknown lebales for 2-way classification
...
def collate_fn(data_bunch):
    ...
    for key in list(dict_data_bunch.keys()):
        dict_data_bunch[key] = torch.stack(dict_data_bunch[key])
...
class DocFormer(pl.LightningModule):
    ...
    def __init__(self, config, model_name = "docformer_base_binary", lr=1e-3):
